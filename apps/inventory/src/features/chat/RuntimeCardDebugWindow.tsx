...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div style={{ marginBottom: 16 }}>
      <div style={{ fontWeight: 700, fontSize: 13, borderBottom: '1px solid #444', paddingBottom: 4, marginBottom: 8 }}>
        {title}
      </div>
      {children}
    </div>
    ...
    <div>
      ...
      {lines.length > maxLines && (
        <button
          ...
          style={{ fontSize: 11, background: 'none', border: 'none', color: '#6ea8fe', cursor: 'pointer', padding: 0 }}
        >
...
export function RuntimeCardDebugWindow() {
  ...
  const td: React.CSSProperties = { padding: '3px 8px', fontSize: 11, borderBottom: '1px solid #333', verticalAlign: 'top' };
  const th: React.CSSProperties = { ...td, fontWeight: 700, background: '#2a2a3e', position: 'sticky', top: 0 };

  return (
    <div style={{ padding: 12, fontFamily: 'monospace', fontSize: 12, color: '#ddd', overflow: 'auto', height: '100%' }}>
      ...
      <Section title={`🃏 Runtime Card Registry (${registryCards.length})`}>
        {registryCards.length === 0 ? (
          <div style={{ fontSize: 11, color: '#777' }}>No runtime cards registered yet.</div>
        ) : (
          registryCards.map(card => (
            <div key={card.cardId} style={{ marginBottom: 12, border: '1px solid #444', borderRadius: 4, padding: 8 }}>
              <div>
      ...
      <Section title={`🗃 Artifacts with Runtime Cards (${runtimeArtifacts.length})`}>
        {runtimeArtifacts.length === 0 ? (
          <div style={{ fontSize: 11, color: '#777' }}>No artifacts with runtime card code.</div>
        ) : (
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr>
              ...
              {runtimeArtifacts.map(a => (
                <tr key={a.id}>
                  ...
                  <td style={td}>
                    ...
                    {a.injectionError && <div style={{ fontSize: 10, color: '#e74c3c', marginTop: 2 }}>{a.injectionError}</div>}
                  </td>
      ...
      <Section title={`⚙️ Plugin Sessions (${Object.keys(sessions).length})`}>
        {Object.keys(sessions).length === 0 ? (
          <div style={{ fontSize: 11, color: '#777' }}>No active plugin sessions.</div>
        ) : (
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr>
              ...
              {Object.entries(sessions).map(([sid, s]) => (
                <tr key={sid}>
                  ...
                  <td style={td}>
                    ...
                    {s.error && <div style={{ fontSize: 10, color: '#e74c3c', marginTop: 2 }}>{s.error}</div>}
                  </td>
                  <td style={td}>
                    {Object.keys(s.cardState ?? {}).map(cid => (
                      <div key={cid}><code style={{ fontSize: 10 }}>{cid}</code></div>
                    ))}
                    {Object.keys(s.cardState ?? {}).length === 0 && <span style={{ color: '#777' }}>—</span>}
                  </td>
                </tr>
              ))}
            </tbody>
