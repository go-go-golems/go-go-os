...
- For `reasoning-summary`, do **not** emit `llm.thinking.delta` if thinking is already ended for that message id.
- Alternative equivalent: map `reasoning-summary` to non-streaming event kind (e.g., `llm.thinking.summary`) and projector updates content without `streaming: true`.
