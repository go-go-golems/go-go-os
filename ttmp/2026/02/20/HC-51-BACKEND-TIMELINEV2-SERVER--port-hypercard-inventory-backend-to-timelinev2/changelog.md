# Changelog

## 2026-02-20

- Initial workspace created


## 2026-02-20

Implemented TimelineV2 backend cutover in pinoweb hypercard timeline handlers (commit f833702): replaced removed TimelineEntityV1 wrappers with TimelineEntityV2 kind+props projection and validated build/tests/startup command.

### Related Files

- /home/manuel/workspaces/2026-02-14/hypercard-add-webchat/2026-02-12--hypercard-react/go-inventory-chat/internal/pinoweb/hypercard_events.go — Code fix for backend startup blocker


## 2026-02-20

Added design and detailed diary docs, checked off completed migration tasks, and left explicit bootstrap-registration follow-up task open.

### Related Files

- /home/manuel/workspaces/2026-02-14/hypercard-add-webchat/2026-02-12--hypercard-react/ttmp/2026/02/20/HC-51-BACKEND-TIMELINEV2-SERVER--port-hypercard-inventory-backend-to-timelinev2/design-doc/01-timelinev2-backend-port-plan.md — Migration rationale and implementation plan
- /home/manuel/workspaces/2026-02-14/hypercard-add-webchat/2026-02-12--hypercard-react/ttmp/2026/02/20/HC-51-BACKEND-TIMELINEV2-SERVER--port-hypercard-inventory-backend-to-timelinev2/reference/01-diary.md — Detailed implementation diary with commands and errors


## 2026-02-20

Added incident postmortem document capturing TimelineV2 backend startup failure, technical root cause (stale TimelineEntityV1 app handlers), remediation details, and prevention follow-ups.

### Related Files

- /home/manuel/workspaces/2026-02-14/hypercard-add-webchat/2026-02-12--hypercard-react/ttmp/2026/02/20/HC-51-BACKEND-TIMELINEV2-SERVER--port-hypercard-inventory-backend-to-timelinev2/design-doc/02-postmortem-timelinev2-backend-cutover-incident.md — New incident postmortem


## 2026-02-20

Uploaded HC-51 postmortem to reMarkable as PDF: /ai/2026/02/20/HC-postmortems/02-postmortem-timelinev2-backend-cutover-incident

### Related Files

- /home/manuel/workspaces/2026-02-14/hypercard-add-webchat/2026-02-12--hypercard-react/ttmp/2026/02/20/HC-51-BACKEND-TIMELINEV2-SERVER--port-hypercard-inventory-backend-to-timelinev2/design-doc/02-postmortem-timelinev2-backend-cutover-incident.md — Source markdown uploaded to reMarkable


## 2026-02-24

Ticket closed


## 2026-02-24

Ticket closed

