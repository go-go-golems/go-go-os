...
- [x] [Impl C2] Ensure EventViewer raw-envelope emission happens at transport ingress before projection
- [x] [Impl C1] Add conversation session transport manager with hydrate-buffer-replay flow and seq/stream_id retention
- [x] [Impl D1] Refactor inventory store wiring to use timeline reducer and new chat runtime path
- [x] [Impl E1] Keep artifacts slice as derived index from timeline tool_result custom kinds and preserve open/edit actions
- [x] [Impl D2] Replace InventoryChatWindow event switch with registry-driven timeline entity handling and entity-first rendering
- [x] [Impl F1] Remove legacy synthetic timeline widget-message path from runtime and update affected stories/tests
- [x] [Impl F2] Run typecheck/tests, fix regressions, and update HC-51 diary/changelog per commit step
