...
function MetadataTable({ item, showRawData }: { item: TimelineWidgetItem; showRawData?: boolean }) {
...
export interface HypercardTimelineWidgetProps {
  items: TimelineWidgetItem[];
  debug?: boolean;
}

export function HypercardTimelineWidget({ items, debug }: HypercardTimelineWidgetProps) {
  ...
  if (items.length === 0) {
    return (
      <div data-part="hypercard-timeline-widget-empty" style={{ fontSize: 11, opacity: 0.75 }}>
        Waiting for events...
      </div>
    );
  }
  ...
  return (
    <div data-part="hypercard-timeline-widget" style={panelStyle}>
      {items.map((item) => {
            ...
            <div>
              <div style={{ display: 'flex', alignItems: 'center', flexWrap: 'wrap', gap: 4 }}>
                {expandable && (
                  <button
                    type="button"
                    data-part="hypercard-timeline-expand-toggle"
                ...
                {kindLabel(item.kind) && (
                  <span data-part="hypercard-timeline-kind" style={chipStyle}>
                    {kindLabel(item.kind)}
                  </span>
                )}
                {item.template && (
                  <span data-part="hypercard-timeline-template" style={{ ...chipStyle, opacity: 0.7 }}>
                    {item.template}
                  </span>
                )}
              </div>
              ...
              {showMeta && <MetadataTable item={item} showRawData={!expanded} />}
            </div>
          </div>
        );
      })}
